...
def book_flight(flight_no: str, passenger_note: str) -> dict:
    ...
    return {"status": "CONFIRMED", "flight_no": flight_no,
            "confirmation": f"TRV-{code}", "note": passenger_note}
...
def book_trip_flight(itinerary: dict, weather: dict) -> dict:
    ...
    prompt = (
        f"Book a flight from {itinerary['origin']} to "
        f"{itinerary['destination']} departing {itinerary['depart_date']}. "
        f"Weather advisory: {weather['advisory']}. "
